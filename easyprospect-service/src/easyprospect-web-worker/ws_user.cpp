    ...
    namespace service
    {
        namespace web_worker
        {
            ...
            protected:
               ...
               //template<typename T>
               // typename std::enable_if<std::is_same<T, plain_ws_session_impl>::value, boost::container::flat_set<shared::ws_session_base<plain_ws_session_impl>*>>::type
               // get_sessions()
               // {
               ...
               // template<typename T>
               // typename std::enable_if<std::is_same<T, ssl_ws_session_impl>::value, boost::container::flat_set<shared::ws_session_base<ssl_ws_session_impl>*>>::type
               // get_sessions()
               // {
            ...
            public:
               ...
               //    
               // void   dispatch(shared::rpc_call& rpc, shared::user& u, ws_session_t& sess);
            ...
            public:
                ...
                void
                    on_insert(shared::user& u, shared::ws_session_t& sess)
                {
                }

               
                void
                    on_erase(shared::user& u, shared::ws_session_t& sess)
                {
                }

               
                void
                    on_dispatch(shared::rpc_call& rpc, shared::user& u, shared::ws_session_t& ses)
                {
                    if (rpc.method == "say")
                    {
                        do_say(rpc, u, ses);
                ...
                void
                    do_say(shared::rpc_call& rpc, shared::user& u, shared::ws_session_t& ses)
                {
                    checked_user(rpc, u, ses);
                    if (!is_joined(u, ses))
                        rpc.fail("not in channel");
                    auto const& text =
                        shared::checked_string(rpc.params, "message");
                    {
                        // broadcast: say
                        nlohmann::json jv;
                        jv["verb"] = "say";
                ...
                void
                    do_slash(shared::rpc_call& rpc, shared::user& u, shared::ws_session_t& ses)
                {
                    checked_user(rpc, u, ses);
            ...
            public:
                channel_list_impl(application_impl& srv) :
                    srv_(srv),
                    next_uid_(1000)
                    , next_cid_(1000)
                {
            ...
            void channel::on_insert(shared::user& u, shared::ws_session_t& sess)
            {
                // TODO: KP. Add user, channel and sess to hash
                throw std::logic_error("Unimplemenentated");

            }

            void channel::on_erase(shared::user& u, shared::ws_session_t& sess)
            {
                // TODO: KP. Remove user, channel and sess to hash
                throw std::logic_error("Unimplemenentated");
            }

            void channel::on_dispatch(shared::rpc_call& rpc, shared::user& u, shared::ws_session_t& sess)
            {
                throw std::logic_error("Unimplemenentated");
            }
            ...
            std::unique_ptr<channel_list_impl>
                make_channel_list(web_worker::application_impl& srv)
            {
            ...
            class application_impl
                : public shared::application_impl_base,
            ...
            void
                make_blackjack_service(
                    web_worker::application_impl& srv);
            ...
            protected:
                ...
                {
                    auto const& name =
                        shared::checked_string(rpc.params, "name");
                    if (name.size() > 20)
            ...
            std::unique_ptr<shared::server> make_server(config::easyprospect_config_service_core curr_config)
            {
                ...
                std::unique_ptr<application_impl> srv;
                {
                    ...
                    {
                        // Create the server
                        srv = boost::make_unique<application_impl>(
                            curr_config);
                    }
...
//------------------------------------------------------------------------------
            ...
            public:
                ...
                std::vector<seat> get_seat() const { return seat_; }
            private:
                state s_ = state::wait;
            ...
            public:
                ...
                void
                    bet(shared::user& u, beast::error_code& ec)
                {
                    auto const i = find(u);
                    if (!i)
                    {
            ...
            public:
                explicit
                    table(web_worker::application_impl& srv)
                    : channel(
                        3,
                        "Blackjack",
                        srv.channel_list())
            ...
            private:
                ...
                void
                    do_insert(boost::shared_ptr<shared::ws_session_t> sp)
                {
                    nlohmann::json jv;
                ...
                void
                    do_erase(shared::user& u, shared::ws_session_t& sess)
                {
