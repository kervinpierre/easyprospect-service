...
boost::program_options::options_description easyprospect_config_server_shell::get_options(
    easyprospect_config_server_shell& config)
{
    auto desc = easyprospect_config_cmd::get_options(config);

    desc.add_options()(
        "worker", boost::program_options::value<bool>(), "Whether this process is a worker process. Boolean.");
    ...
    desc.add_options()("webroot-dir", boost::program_options::value<std::string>(), "Webroot directory");

    desc.add_options()("num-threads", boost::program_options::value<std::string>(), "Number of threads");
    ...
    desc.add_options()("listen-file", boost::program_options::value<std::string>(), "Where we install listen file");

    desc.add_options()(
        "listen-exe-dir", boost::program_options::value<std::string>(), "Directory for storing generated listen files");
    return desc;
}
...
easyprospect_config_worker_core_builder easyprospect_config_worker_shell::init_args(int test_argc, char* test_argv[])
{
    ...
    boost::program_options::options_description opts = easyprospect_config_service_shell::get_options(cnf);
