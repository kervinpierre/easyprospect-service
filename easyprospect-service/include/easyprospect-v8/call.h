...
template<typename F>
using isolate_arg_call_traits = CallFromTraits<F, 1>;

template<typename F, size_t Offset = 0>
struct v8_args_call_traits : CallFromTraits<F, Offset>
{
    template<size_t Index, typename Traits>
...
using is_direct_args = std::integral_constant<bool,
    CallFromTraits<F>::arg_count == (Offset + 1) &&
    std::is_same<typename CallFromTraits<F>::template arg_type<Offset>,
        v8::FunctionCallbackInfo<v8::Value> const&>::value>;

template<typename F>
using is_first_arg_isolate = std::integral_constant<bool,
    CallFromTraits<F>::arg_count != 0 &&
    std::is_same<typename CallFromTraits<F>::template arg_type<0>,
        v8::Isolate*>::value>;
