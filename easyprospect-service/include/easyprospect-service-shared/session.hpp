    ...
    namespace service
    {
        namespace shared
        {
            ...
            protected:
             ...
             //   std::function<void(rpc_call&, user&)> dispatch;
            ...
            class ws_session_t :
                public boost::enable_shared_from
            {
                std::unique_ptr<plain_http_session_impl> plain_ptr_;
                std::unique_ptr<ssl_http_session_impl>   ssl_ptr_;

            public:
                ws_session_t( application_impl_base& srv, listener& lst,
                              stream_type stream,
                              endpoint_type ep, flat_storage storage );
                ws_session_t( application_impl_base& srv, listener& lst,
                              asio::ssl::context& ctx,
                              stream_type stream, endpoint_type ep,
                              flat_storage storage );
            ...
            class plain_ws_session_impl
                : public shared::ws_session_base<plain_ws_session_impl>
            {
            ...
            http_session_base<Derived>::http_session_base(
                application_impl_base& srv, listener& lst, endpoint_type ep,
                flat_storage storage ) : srv_( srv )
                , lst_( lst )
                , ep_( ep )
            ...
            void handle_request( boost::optional<boost::filesystem::path> doc_root,
                                 http::request<Body, http::basic_fields<Allocator>>&& req,
                                 Send&& send );

            template <class Derived>
            void http_session_base<Derived>::operator()( beast::error_code ec,
                                                         std::size_t  bytes_transferred,
                                                         bool need_eof )
            {
            ...
            template <class Derived>
            void ws_session_base<Derived>::operator()(
                beast::error_code ec,
                std::size_t bytes_transferred )
            {
#include <boost/asio/yield.hpp>
